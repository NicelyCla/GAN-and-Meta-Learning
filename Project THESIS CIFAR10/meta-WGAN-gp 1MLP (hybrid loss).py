...
def train_generator():
    label = torch.full((batch_size,), 1.0, dtype=torch.float, device=device)#real label
    ...
    err_gen_fake_domain = criterion(gen_fake_domain,label)
    err_gen_fake_domain.backward(retain_graph=True)
...
def train_critic_on_task(single_loader, train_loader):
    for epoch in range(NUM_EPOCHS):
        ...
        for _ in range(CRITIC_ITERATIONS):
            ...
            #train with real
            label = torch.full((cur_batch_size,), real_label, dtype=torch.float, device=device)#real label
            ...
            err_critic_real_domain = criterion(critic_real_domain,label)
            err_critic_real_domain.backward(retain_graph=True)
            ...
            err_critic_fake_domain = criterion(critic_fake_domain,label)
            err_critic_fake_domain.backward(retain_graph=True)
            ...
            loss_critic = (
                ...
                #(err_critic_real_domain - err_critic_fake_domain)
        ...
        loss_mlp = -torch.mean(gen_fake)
        
        #loss_mlp = -(torch.mean(gen_fake) + err_gen_fake_domain)
...
for iteration in tqdm(range(niterations)):
    ...
    #TEST    
    if (iteration%5 == 0 or iteration+1==niterations) and iteration != 0:
