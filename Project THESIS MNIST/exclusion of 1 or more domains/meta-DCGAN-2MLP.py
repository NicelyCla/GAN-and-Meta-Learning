...
esclusi = [9]
if(len(esclusi) == 9):
    print("Puoi escludere al massimo 8 domini su 10")
    exit(-1)
...
for img, label in train_loader:
    if not(label.item() in esclusi): #escludo alcuni domini
        if (not(kShot_bool)):
...
#Pulisco il train loader da dai domini esclusi:
#############################################################
loaders_list=[[]]
label_cat=[]
img_cat=[]
...
for i,data_single_class in enumerate(loader_single_class):
    for j,(img,label) in enumerate(data_single_class):
        
        if not(label[0].item() in esclusi):
            label_cat.append(label)
            img_cat.append(img)
...
for data_list in loaders_list:
    ...
    # ogni volta che si itera il dataset, viene fatto a random
    #b_size è il batch_size per la modalità lenta anche qualora non ci sia il kshot attivato
    domain_clear.append(DataLoader(
            dataset,
            sampler=RandomSampler(dataset),
            batch_size=batch_size
        )
    )
train_loader = domain_clear[0]
######################################################################


#creo un test_loader pulito per l'antidominio
#############################################################
loaders_list_test=[[]]
label_cat=[]
img_cat=[]
...
for i,data_single_class in enumerate(loader_single_class_test):
    for j,(img,label) in enumerate(data_single_class):
...
loaders_list_test[0].append((img_cat, label_cat))
#i tensori dentro la lista vengono tutti concatenati
label_cat = torch.cat(label_cat, dim=0)
img_cat = torch.cat(img_cat, dim=0)
...
#create antidomain
domain_clear_test = []
for data_list in loaders_list_test:
    ...
    # ogni volta che si itera il dataset, viene fatto a random
    #b_size è il batch_size per la modalità lenta anche qualora non ci sia il kshot attivato
    domain_clear_test.append(DataLoader(
            dataset,
            sampler=RandomSampler(dataset),
            batch_size=batch_size
        )
    )
test_loader_clear = domain_clear_test[0]
######################################################################
...
def train_GAN_on_task(model_G, model_D, optimizer_G, optimizer_D, single_loader, train_loader, train):
    ...
    for i in range(innerepochs):
        ...
        output, out_cls, feat = model_D(real_cpu)
        output = output.view(-1)
        out_cls = out_cls.view(-1)
        ...
        out_real, out_dom, _  = model_D(out_gen)
        ...
        x_out = model_G(z_cls)
        out_real, out_dom, _ = model_D(x_out.detach())
        label.fill_(real_label)
...
for iteration in tqdm(range(niterations)):
    ...
    random = randrange(len(loader_single_class))
    ...
    if plot and (iteration==0 or iteration%1000 == 0 or iteration+1==niterations):
        ...
        for n in range(NUM_LABELS):
            ...
            err_D, err_MLP, err_MLP_cls, label, cls_cond = train_GAN_on_task(model_G, model_D, optimizer_G, optimizer_D, single_loader_test, test_loader_clear, train=False)
            err_G = train_G(model_G, label, optimizer_G, cls_cond)
